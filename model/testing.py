...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=()):
        ...
        self.cls = torch.nn.ModuleList(torch.nn.Sequential(Conv(x, c1, 3),
                                                           Conv(c1, c1, 3),
                                                           torch.nn.Conv2d(c1, self.nc, 3, padding=1)) for x in filters)
        self.box = torch.nn.ModuleList(torch.nn.Sequential(Conv(x, c2, 3),
                                                           Conv(c2, c2, 3),
                                                           torch.nn.Conv2d(c2, 4 * self.ch, 3, padding=1)) for x in filters)
...
class YOLO2Stream_testing(torch.nn.Module):
    def __init__(self, width, depth, num_classes, backbone3D, fusion_module, pretrain_yolo=None, pretrain_path=None):
        ...
        out_2D = self.fpn(self.net(dummy_img2D))
    ...
    def forward(self, clips):
        ...
        ft_2D = self.fpn(self.net(key_frames))
...
def yolo_v8(num_classes, ver, backbone_3D, fusion_module, pretrain_path=None):
    ...
    return YOLO2Stream_testing(width, depth, num_classes, backbone3D, fusion_module, pretrain_yolo, pretrain_path)
